...
SELECT DATE_FORMAT(o.date, "%Y-%M") AS month, SUM(rd.revenue) AS revenue
FROM orders AS o
...
SELECT ROUND(SUM(od.quantity) / COUNT(DISTINCT o.date),0) AS Avg_pizza_sold_per_day
FROM order_details od
...
SELECT cd.category, CONCAT(ROUND((SUM(cd.price * od.quantity) / 
                    ...
                    JOIN order_details AS od ON p.pizza_id = od.pizza_id)) * 100,0),"%") AS revenue_contribution
FROM category_data AS cd
JOIN order_details AS od ON cd.pizza_id = od.pizza_id
GROUP BY cd.category
...
date_data AS (SELECT MONTH(o.date) AS month_number, DATE_FORMAT(o.date, "%Y-%M") AS month_name, SUM(od.revenue) AS revenue
FROM order_data AS od
JOIN orders AS o ON od.order_id = o.order_id
